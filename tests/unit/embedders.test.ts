...
import { HashTextEmbedder, OpenAITextEmbedder } from "../../packages/memory/src/embedding/embedders";
...
describe("OpenAITextEmbedder — constructor", () => {
  ...
  it("throws when apiKey is missing", () => {
    expect(() => new OpenAITextEmbedder({ model: "text-embedding-3-small", baseURL: "http://x" })).toThrow();
  });

  it("throws when baseURL is missing", () => {
    expect(() => new OpenAITextEmbedder({ model: "text-embedding-3-small", apiKey: "k" })).toThrow();
  });

  it("constructs successfully with all required fields", () => {
    expect(() => new OpenAITextEmbedder({
