...
describe("approxTokenLen()", () => {
  it("counts English words", () => {
    expect(approxTokenLen("hello world")).toBe(2);
  });
  it("counts CJK chars", () => {
    expect(approxTokenLen("中文")).toBeGreaterThanOrEqual(2);
  });
  it("returns 0 for empty", () => {
    expect(approxTokenLen("")).toBe(0);
  });
});

describe("detectLang()", () => {
  it("returns zh for CJK text", () => {
    expect(detectLang("这是中文文本测试语言检测")).toBe("zh");
  });
...
describe("isMarkitdownSupportedFormat()", () => {
  it("returns true for .pdf", () => {
    expect(isMarkitdownSupportedFormat("file.pdf")).toBe(true);
  });
  it("returns true for .md", () => {
    expect(isMarkitdownSupportedFormat("README.md")).toBe(true);
  });
  it("returns true for .ts", () => {
    expect(isMarkitdownSupportedFormat("src/index.ts")).toBe(true);
  });
  it("returns false for unknown ext", () => {
    expect(isMarkitdownSupportedFormat("file.xyz")).toBe(false);
  });
});

describe("fallbackTextReader()", () => {
  it("returns empty for non-existent", () => {
    expect(fallbackTextReader("/no/such/file.txt")).toBe("");
  });
  it("reads existing file", () => {
    const f = path.join(os.tmpdir(), `fb-${Date.now()}.txt`);
    fs.writeFileSync(f, "hello", "utf-8");
    try {
      expect(fallbackTextReader(f)).toBe("hello");
    } finally {
      fs.unlinkSync(f);
    }
  });
...
describe("convertToMarkdown()", () => {
  it("returns empty for non-existent", () => {
    expect(convertToMarkdown("/no/file.txt")).toBe("");
  });
  it("reads file without adapter", () => {
    const f = path.join(os.tmpdir(), `cm-${Date.now()}.txt`);
    fs.writeFileSync(f, "content", "utf-8");
    try {
      expect(convertToMarkdown(f)).toBe("content");
    } finally {
      fs.unlinkSync(f);
    }
  });
  it("uses adapter for supported format", () => {
    ...
    try {
      expect(convertToMarkdown(f, adapter)).toBe("converted");
    } finally {
      fs.unlinkSync(f);
    }
  });
  it("falls back when adapter throws", () => {
    ...
    try {
      expect(convertToMarkdown(f, adapter)).toBe("fallback");
    } finally {
      fs.unlinkSync(f);
    }
  });
  it("falls back when adapter returns empty", () => {
    ...
    try {
      expect(convertToMarkdown(f, adapter)).toBe("original");
    } finally {
      fs.unlinkSync(f);
    }
  });
