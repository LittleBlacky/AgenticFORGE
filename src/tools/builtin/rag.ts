...
} from "../../memory/rag/pipeline";
import { createDefaultVectorStore } from "../../memory/rag/storeFactory";
import { createDefaultTextEmbedder } from "../../memory/embedding/factory";
...
export class RagTool extends Tool {
  ...
  private initComponents(): void {
    try {
      ...
      console.log(`[RagTool] ✅ 初始化成功  namespace=${this.ragNamespace}  collection=${this.collectionName}`);
    } catch (e) {
  ...
  getParameters(): ToolParameter[] {
    return [
      { name: "action",                 type: "string",  required: true,  default: null,      description: "操作类型：add_document | add_text | ask | search | stats | clear" },
      { name: "file_path",              type: "string",  required: false, default: null,      description: "文档文件路径（支持 PDF、Word、Excel、PPT、图片、音频等多种格式）" },
      { name: "text",                   type: "string",  required: false, default: null,      description: "要添加的文本内容" },
      { name: "question",               type: "string",  required: false, default: null,      description: "用户问题（用于智能问答）" },
      { name: "query",                  type: "string",  required: false, default: null,      description: "搜索查询词（用于基础搜索）" },
      { name: "namespace",              type: "string",  required: false, default: "default", description: "知识库命名空间，用于多项目隔离（默认：default）" },
      { name: "document_id",            type: "string",  required: false, default: null,      description: "文档 ID（不传则自动生成）" },
      { name: "limit",                  type: "number",  required: false, default: 5,         description: "返回结果数量（默认：5）" },
      { name: "chunk_size",             type: "number",  required: false, default: 800,       description: "分块大小（token 数，默认：800）" },
      { name: "chunk_overlap",          type: "number",  required: false, default: 100,       description: "分块重叠大小（token 数，默认：100）" },
      { name: "min_score",              type: "number",  required: false, default: 0.1,       description: "最低相关度分数（默认：0.1）" },
      { name: "enable_advanced_search", type: "boolean", required: false, default: true,      description: "是否启用高级检索（MQE + HyDE，默认：true）" },
      { name: "include_citations",      type: "boolean", required: false, default: true,      description: "是否包含引用来源（默认：true）" },
      { name: "max_chars",              type: "number",  required: false, default: 1200,      description: "上下文最大字符数（默认：1200）" },
      { name: "confirm",                type: "boolean", required: false, default: false,     description: "清空操作安全确认（必须设置为 true）" },
    ];
  }
  ...
  async _search(
    ...
    try {
      ...
      const results = enableAdvancedSearch
        ? await pipeline.searchAdvanced(query, limit, true, true, minScore > 0 ? minScore : undefined)
    ...
    try {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        if (content) {
          ...
          if (includeCitations) {
            citations.push({ index: i + 1, source: path.basename(source), score: hit.score });
          }
        }
      }
  ...
  async _clearKnowledgeBase(confirm = false, namespace = "default"): Promise<string> {
    ...
    try {
      ...
      const success  = typeof store.clearCollection === "function"
