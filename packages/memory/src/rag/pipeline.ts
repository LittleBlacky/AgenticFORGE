...
export function postProcessPdfText(text: string): string {
  ...
  for (const raw of lines) {
    ...
    if (["github", "project", "forks", "stars", "language"].includes(lower)) continue;
    cleaned.push(line);
  }
  return cleaned.join("\n");
}
...
export function chunkParagraphs(
  ...
  if (cur.length > 0) chunks.push(emitChunk(cur));
  return chunks;
}
...
export function buildGraphFromChunks(
  ...
  for (const chunk of chunks) {
    ...
    const sourcePath = typeof meta.source_path === "string" ? meta.source_path : undefined;
...
export async function indexChunks(options: IndexChunksOptions): Promise<void> {
  ...
  for (let i = 0; i < processedTexts.length; i += batchSize) {
    ...
    vectors.push(...normalize2DVectors(raw, dimension, part.length));
  }
...
export async function searchVectorsExpanded(
  ...
  for (const q of uniq) {
    ...
    for (const hit of hits) {
      ...
      if (!prev || hit.score > prev.score) agg.set(mid, hit);
    }
  }
...
export function computeGraphSignalsFromPool(
  ...
  if (maxV > 0) for (const key of Object.keys(graphSignal)) graphSignal[key] /= maxV;
  return graphSignal;
}
...
function normalize1DVector(raw: number[] | number[][], dimension: number): number[] {
  const vector = Array.isArray(raw[0]) ? (raw as number[][])[0] : (raw as number[]);
  const out = vector.map((n) => Number(n));
  if (out.length < dimension) out.push(...new Array(dimension - out.length).fill(0));
  if (out.length > dimension) out.length = dimension;
  return out;
}
