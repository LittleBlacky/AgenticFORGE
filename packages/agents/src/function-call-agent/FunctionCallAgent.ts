...
export class FunctionCallAgent extends Agent {
    ...
    for (const [name, fnTool] of fnMap?.entries() ?? []) {
      ...
      if (fnTool.schema) {
        try {
          ...
          // ignore and fallback to default schema
        }
      }
  ...
  private async executeToolCall(
    ...
    try {
      const typedArguments = this.convertParameterTypes(toolName, argumentsDict);
      return await this.toolRegistry.execute(toolName, typedArguments);
    } catch (error) {
    ...
    while (currentIteration < iterationsLimit) {
      ...
      if (toolCalls.length > 0) {
        ...
        for (const toolCall of toolCalls) {
          ...
          const args = FunctionCallAgent.parseFunctionCallArguments(argsText);
