...
export class SkillAgent extends Agent {
  ...
  constructor(params: {
    ...
    this.routerPromptTemplate = params.routerPromptTemplate ?? [
  ...
  addSkill(skill: IAgentSkill): void { this.skillRegistry.register(skill); }
  removeSkill(name: string): boolean { return this.skillRegistry.unregister(name); }
  listSkills(): string[] { return this.skillRegistry.list(); }
  ...
  private async routeToSkill(query: string): Promise<IAgentSkill | undefined> {
    ...
    return this.skillRegistry.get(raw)
  ...
  async runSkill(skillName: string, query: string, metadata?: Record<string, unknown>): Promise<SkillResult> {
  ...
  async *streamRun(
    ...
    if (!skill) {
      ...
      let full = "";
      for await (const chunk of this.llm.streamThink(msgs, options?.temperature)) { full += chunk; yield chunk; }
      this.addMessage(new Message({role: "user", content: inputText}));
    ...
    const context: SkillContext = {
      ...
      history: this.history.map((m) => ({role: m.role as "user" | "assistant", content: m.content})),
    };
  ...
  private async runSkillInternal(
    ...
    if (options?.skillName) {
      skill = this.skillRegistry.get(options.skillName);
      if (!skill) throw new Error(`Skill "${options.skillName}" not found. Available: ${this.skillRegistry.list().join(", ")}`);
    } else {
      skill = await this.routeToSkill(inputText);
    }
    if (!skill) {
