...
```209:295:src/context/ContextBuilder.ts
...
- `build` 阶段会创建 **按文本内容缓存** 的 token 计数器，避免重复计算。
...
- **模型名自动推断**：
  - `Tokenizer` 支持 `model` 参数，可自动映射到合适的 encodingName。
- **lazy 单例缓存**：
  - `encodingCache` 按编码名称缓存 `js-tiktoken` 实例。
- **包级 token 计数缓存**：
  - `build` 阶段创建 `cachedCounter`，以文本内容为 key 缓存计数，避免重复计算。
- **MMR 打分**：
...
## 6. 可靠性与降级策略
- **token 计数器缺失**：自动使用 `roughCountTokens`（粗略估算）。
- **js-tiktoken 不可用**：自动回退到粗略估算并提示警告。
- **MMR 关闭**：若 `enableMmr=false`，退回原有相关性优先选择。
- **token 预算不足**：`compress` 会按行截断保结构。

## 7. 局限与演进建议
- 当前 MMR 相似度为词集合重叠，语义相似度较弱，可升级为向量相似度。
- 模型名 → encodingName 的映射仍是规则表，建议长期维护或外部配置化。
- token 缓存当前以文本为 key，极长文本可能带来内存占用，可考虑 LRU。
