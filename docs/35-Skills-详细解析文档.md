...
```mermaid
flowchart TD
    A["LLM 返回 raw 字符串"] --> B["registry.get(raw) 精确匹配"]
    B -->|命中| Z["返回 IAgentSkill"]
    B -->|失败| C["name.startsWith(raw) 前缀匹配"]
    C -->|命中| Z
    C -->|失败| D["raw.includes(name) 包含匹配"]
    D -->|命中| Z
    D -->|失败| E["返回 undefined → fallback LLM"]
```
...
```mermaid
flowchart TD
    A["run(query, options?)"] --> B{options.skillName?}
    B -->|有| C["registry.get(skillName)"]
    C -->|找到| G["skill.execute(context, llm)"]
    C -->|找不到| ERR["throw Error"]
    B -->|无| D["routeToSkill(query)"]
    D -->|undefined| F["fallback llm.think() → {output}"]
    D -->|IAgentSkill| G
    G --> H["返回 SkillResult"]
    F --> H
```
...
```mermaid
flowchart TD
    A["原始 Markdown 文本"] --> B["正则匹配 --- 块"]
    B -->|不匹配| C["从 H1 推断 name/description
全文作为 body"]
    B -->|匹配| D["逐行解析 yamlBlock"]
    D --> E{值类型}
    E -->|true/false| F["转换为 boolean"]
    E -->|其余| G["去除引号，trim"]
    F --> H["body = frontmatter 后内容 trim"]
    G --> H
    C --> I["返回 { frontmatter, body }"]
    H --> I
```
...
```mermaid
flowchart TD
    A["fromDirectory(dir)"] --> B["resolve 转绝对路径"]
    B --> C["walk(absDir)"]
    C --> D["readdir(current)"]
    D -->|失败| E["静默跳过"]
    D -->|成功| F["遍历 entries"]
    ...
    G -->|文件| H{isSkillFile?}
    H -->|否| I["跳过"]
    H -->|是| J["MarkdownSkill.fromFile()"]
    J -->|成功| K["push 到 skills[]"]
    J -->|失败| L["console.warn 继续"]
```
