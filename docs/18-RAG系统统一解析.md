...
graph TD
  ...
  I1[输入: Document] --> A
  O1[输出: DocumentChunks] --> C
```

- 参数说明: `chunkSize/chunkOverlap/separators` 影响分块策略。
- 输入示例: `Document{content:"...", metadata:{...}}`
- 输出示例: `DocumentChunk[]`
...
- 输入示例: `paths=["docs/a.md","docs/b.pdf"]`
- 输出示例: `context + citations`
...
graph TD
  ...
  P1[参数: markitdownAdapter] --> B
  I1["输入: paths[]"] --> A
  O1["输出: LoadedDocument[]"] --> E
```

- 输入示例: `paths=["docs/a.md"]`
- 输出示例: `[{filePath:"docs/a.md", docId:"...", paragraphs:[...]}]`
...
graph TD
  ...
  P1[参数: chunkSize] --> B
  P2[参数: chunkOverlap] --> B
  I1[输入: paragraphs] --> A
  O1["输出: RagChunk[]"] --> E
```

- 输入示例: `paragraphs=[{content:"..."}]`, `chunkSize=800`, `chunkOverlap=100`
- 输出示例: `RagChunk[] (含 content_hash/start/end)`
...
graph TD
  ...
  P1[参数: enableMqe/mqeExpansions] --> B
  P2[参数: enableHyde] --> C
  P3[参数: topK/candidatePoolMultiplier] --> E
  I1[输入: query] --> A
  O1[输出: VectorSearchHit[]] --> F
```

- 参数说明: `enableMqe`/`mqeExpansions` 控制扩展数量，`enableHyde` 控制假设答案扩展，`topK` 与 `candidatePoolMultiplier` 决定召回池规模。
- 输入示例: `query="RAG 分块"`, `enableMqe=true`, `mqeExpansions=2`, `enableHyde=true`
- 输出示例: `[{id:"...", score:0.82, metadata:{...}}, ...]`
...
- `mergeSnippets`：按长度截断拼接
- `expandNeighborsFromPool`：补齐同文档邻接片段
...
graph TD
  ...
  I1[输入: VectorSearchHit[]] --> A
  O1[输出: context + citations] --> F
```

- 输入示例: `[{id:"m1", score:0.7, metadata:{...}}, ...]`
- 输出示例: `"chunk1... [1]\n\nchunk2... [2]\n\nReferences: ..."`

```mermaid
graph TD
  A[computeGraphSignalsFromPool] --> B[同文档密度]
  A --> C[邻近度信号]
  B --> D[归一化]
  C --> D

  P1[参数: sameDocWeight] --> B
  P2[参数: proximityWeight] --> C
  P3[参数: proximityWindowChars] --> C
  O1[输出: graphSignals] --> D
```

- 参数说明: `sameDocWeight` 控制文档密度权重，`proximityWeight` 控制邻近度权重，`proximityWindowChars` 决定距离窗口。
- 输出示例: `{memoryIdA: 0.82, memoryIdB: 0.35}`

```mermaid
graph TD
  A[expandNeighborsFromPool] --> B[按 doc 分组 pool]
  B --> C[邻近扩展]
  C --> D[去重]
  D --> E[合并 + 排序]

  P1[参数: neighbors] --> C
  P2[参数: maxAdditions] --> C
  I1[输入: selected + pool] --> A
  O1[输出: expandedItems] --> E
```

- 参数说明: `neighbors` 控制左右邻接扩展步数，`maxAdditions` 限制新增数量。
- 输入示例: `selected=[chunk2], pool=[chunk1,chunk2,chunk3]`
- 输出示例: `expandedItems=[chunk2,chunk1,chunk3]`

```mermaid
graph TD
  A[mergeSnippets] --> B[累积长度]
  B --> C{超出 maxChars?}
  C -->|是| D[截断并返回]
  C -->|否| E[追加文本]
  E --> B
  ...
  P1[参数: maxChars] --> B
  I1[输入: rankedItems] --> A
  O1[输出: mergedSnippet] --> D
```

- 参数说明: `maxChars` 控制最终合并文本的最大长度。
- 输入示例: `rankedItems = [{content: "A..."}, {content: "B..."}]`
- 输出示例: 合并后文本长度不超过 `maxChars`。
...
```mermaid
graph TD
  A[vector_score] --> C[score = wVector * v + wGraph * g]
  B[graph_score] --> C
  C --> D[排序输出]
```
...
```mermaid
graph TD
  A[normalize1DVector] --> B[截断/补零]
  B --> C[输出固定维度]
  D[normalize2DVectors] --> E[逐条 normalize1DVector]
  E --> F[补足 expectedCount]

  P1[参数: dimension] --> B
  P2[参数: expectedCount] --> F
  I1[输入: raw vectors] --> D
  O1[输出: normalized vectors] --> F
```

- 参数说明: `dimension` 强制向量维度一致，`expectedCount` 确保批次长度一致。
- 输入示例: `raw=[[0.1,0.2]]`, `dimension=4`, `expectedCount=2`
- 输出示例: `[[0.1,0.2,0,0],[0,0,0,0]]`
...
graph LR
  ...
  I1[输入: RagVectorStoreFactoryOptions] --> A
  O1[输出: VectorStoreAdapter] --> B
```

- 参数说明: `backend` 决定后端类型，`qdrantUrl/qdrantApiKey/...` 仅在 qdrant 模式生效。
- 输入示例: `{backend:"qdrant", qdrantUrl:"http://localhost:6333"}`
- 输出示例: `QdrantVectorStore`
...
graph TD
  ...
  I1[输入: CreateRagPipelineOptions] --> A
  O1[输出: RagPipeline] --> A
```

- 参数说明: `ragNamespace/ragUserId/store/embedder/dimension` 影响默认行为。
- 输入示例: `{ragNamespace:"default", ragUserId:"user-123"}`
- 输出示例: `{addDocuments, search, searchAdvanced}`
